...
<head>
    ...
    <title>Antes | Change password</title>
    <!--Animation-->
...
<nav class="navbar navbar-expand-lg navbar-light bg-white fixed-top">
    ...
    <div class="container">
        <h2 class="text-center" style="margin-top: 100px">Change Password</h2>
        <div class="row d-flex flex-column align-items-center mx-3">
            <div class="col-12 col-lg-6">
                <form class="row g-3" action="Includes/changepw.inc.php" method="POST" enctype="multipart/form-data">
                    <div class="col-12">
                        <label for="exampleFormControlLocation" class="form-label">New password</label>
                        <input type="password" class="form-control" id="exampleFormControlInput3" placeholder="New password" name="password">
                    </div>
                    <div class="col-12">
                        <label for="exampleFormControlLocation" class="form-label">Repeat new password</label>
                        <input type="password" class="form-control" id="exampleFormControlInput4" placeholder="Repeat new password" name="r_password">
                    </div>
                    <div class="col-12">
                        <?php 
                        
                        if(isset($_GET["msg"])){
                            if($_GET["msg"]=="notmatch"){
                                echo "<h5 class='text-danger mt-3'>Fields do not match!</h5>";
                            }else if($_GET["msg"]=="emptyfield"){
                                echo "<h5 class='text-danger mt-3'>Fields cannot be empty!</h5>";
                            }else if($_GET["msg"]=="wentwrong"){
                                echo "<h5 class='text-danger mt-3'>Something went wrong, Try again!</h5>";
                            }else if($_GET["msg"]=="updatesuccess"){
                                echo "<h5 class='text-success mt-3'>Password updated successfully.</h5>";
                            }
                        }
                         ?>
                    </div>
                    <div class="col-12">
                        <button type="submit" class="btn btn-primary" name="updatepw">Submit</button>
                    </div>
                </form>
            </div>
        </div>
