...
<nav class="navbar navbar-expand-lg navbar-light bg-white fixed-top">
    ...
    <form action="Includes/add-blog.php" method="POST">
  <div class="form-group">
    <label for="blogtitle">Blog title</label>
    <input type="text" class="form-control" id="blogtitle" name="blogtitle" aria-describedby="emailHelp" placeholder="Enter title">
  </div>
  <div class="form-group mt-5">
    <label for="summernote">Blog content</label>
    <textarea class="form-control" name="blogdesc" id="summernote" rows="3"></textarea>
  </div>
  <div>
    <?php 
    if(isset($_GET["status"])) {
      if($_GET["status"] == "emptyinput") {
        echo "<h5 class='text-danger mt-3'>Please fill the fields!</h5>";
    }else if($_GET["status"]=="error") {
      echo "<h5 class='text-danger mt-3'>An error occured. Try again!</h5>";
    }else if($_GET["status"]=="success") {
      echo "<h5 class='text-success mt-3'>Blog successfully added.</h5>";
      
    }
  }
    ?>
  </div>
  <button type="submit" name="addPost" class="btn btn-primary mt-3">Submit</button>
</form>
    </div>
